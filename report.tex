...
To understand the behavior of operating system bootup, we use perf - a commonly used statistical profiler in Linux - to collect sample data on the execution of a User-mode Linux (UML) and to conduct dynamic program analysis \cite{perf2013}. Since we mainly care about the activities happening at system booting, we use perf to profile a process that does the following tasks:
\begin{enumerate}[(1)]
\item Boot UML;
\item Log in Linux;
\item Halt
...
\section{Appendix C}
This section includes two flame charts. (See next 2 pages)
\begin{figure*}[!htb]
\centering
...
\begin{table}[!htb]
\centering
\caption{List of softwares installed on MicroswiftOS}
\begin{tabular}{|c|c|c|}
\hline
